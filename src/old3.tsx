...
const SlotMachine = () => {
  ...
  const spinReels = () => {
    ...
    if (spinAudioRef.current) {
      spinAudioRef.current.currentTime = 0; // Reset audio
    ...
    // End spin after 2 seconds
    setTimeout(() => {
      setSpinning(false);

      // Stop or pause the audio when spin ends
      if (spinAudioRef.current) {
...
const Reel = ({ targetSymbol, spinning, delay }: ReelProps) => {
  const loopDistance = -symbols.length * 200; // Large loop distance to create a clear spinning effect
  ...
  const { y } = useSpring({
    ...
    to: async (next) => {
      if (spinning) {
        // Continuous spinning effect while `spinning` is true
        while (spinning) {
          await next({ y: loopDistance, config: { tension: 500, friction: 20 } });
          await next({ y: 0, config: { tension: 500, friction: 20 } });
        }
      } else {
        // Smooth stop at the target symbol when `spinning` is false
  ...
  return (
    <ReelWrapper>
      <ReelContainer as={animated.div} style={{ transform: y.to((y) => `translateY(${y}px)`) }}>
        {/* Loop symbols to show continuous spinning */}
        {Array(20)
          .fill(null)
          .map((_, i) => (
